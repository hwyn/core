...
export const registerSeed = createRegisterLoader(SEED_TOKEN);
/**
...
export function createSeedDecorator(options, props) {
    const { slot, aggregation, profile = 'default', protocol, strategy } = options;
    const metadataFactory = props || ((...args) => ({ payload: args.length > 1 ? args : args[0] }));
    return (...args) => {
        return ((target, propertyKey, descriptor) => {
            // Different handling for Class vs Method
            const isClassDecorator = !propertyKey;
            const hostClass = isClassDecorator ? target : target.constructor;
            const computedPropertyKey = isClassDecorator ? EXECUTE_METHOD_NAME : propertyKey;
            // Validation for Class Seeds: Check if 'execute' method exists
            if (isClassDecorator) {
                if (typeof target.prototype[EXECUTE_METHOD_NAME] !== 'function') {
                    throw new Error(`[Seed Decorator] Class '${target.name}' is decorated as a @Seed but does not implement the '${EXECUTE_METHOD_NAME}' method. Please implement the ExecutableSeed interface.`);
                }
                setInjectableDef(target);
            }
            const result = metadataFactory(...args) || {};
            const hasRoute = !!result.route;
            const finalAggregation = result.aggregation || aggregation;
            const effectiveProtocol = result.protocol || protocol;
            if (!effectiveProtocol) {
                throw new Error(`[Seed Decorator] Protocol is required for @${slot}. Please specify a valid protocol (e.g. HTTP_PROTOCOL). Target: ${hostClass === null || hostClass === void 0 ? void 0 : hostClass.name}`);
            }
            if (finalAggregation === 'PROCESS_DEF' && !hasRoute) {
                throw new Error(`[${slot}] PROCESS_DEF seed must provide a route. Target: ${hostClass === null || hostClass === void 0 ? void 0 : hostClass.name}`);
            }
            const desc = Object.assign(Object.assign({ action: InstructionAction.ADD }, result), { hostClass, propertyKey: computedPropertyKey, slotName: slot, aggregation: finalAggregation, profile: result.profile || profile, protocol: result.protocol || protocol, strategy: result.strategy || strategy });
